...
However, researchers preferring Quarto Markdown have no equivalent Python tool. `lintquarto` closes this gap by enabling established Python code quality tools to run on Quarto Markdown files.
...
The core workflow converts each `.qmd` file into a temporary Python file. Where relevant, the converter preserves line numbers, enabling error messages to map directly back to the original Quarto file. The converter handles Quarto-specific syntax including Quarto chunk options, include directives, in-line annotations (e.g., `#<<`) and false positives triggered by chunk boundaries.

After conversion, `lintquarto` runs the selected code quality tool on the temporary Python file, captures its output, and rewrites reported paths to reference the original `.qmd` filename. This approach enables seamless integration. with no modifications to the underlying existing code quality tools, and makes it easy to extend to incorporate new tools.

The package supports all major Python code quality tools:
